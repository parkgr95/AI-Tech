...
maxDV(G,D)=Ex∼pdata[logD(x)]+Ex∼pG[log(1−D(x))]maxDV(G,D)=Ex∼pdata[logD(x)]+Ex∼pG[log(1−D(x))]



이 식을 최대화 하는 D는 D∗G(x)=pdata(x)pdata(x)+pG(x)DG∗(x)=pdata(x)pdata(x)+pG(x)라고 할 수 있습니다. 이것을 다시 generator 입장에 넣었을 때, 다음과 같은 식을 얻을 수 있습니다. 



V(G,D∗G(x))=DKL[pdata,pdata(x)+pG2]+DKL[pG,pdata+pG2]−log4V(G,DG∗(x))=DKL[pdata,pdata(x)+pG2]+DKL[pG,pdata+pG2]−log4
